...
- Modern Steel Type, making Ghost and Dark hit Steel neutrally
...
- Infinite TMs
- Item Expansion, creating real new item IDs from cloned vanilla item behavior with overflow bag storage
- Extra TMs, creating up to 60 configurable-move TM93-TM152 entries through Item Expansion
- Nature Mints, creating 25 expanded mint items that change a Pokemon's actual displayed nature
- Bottle Caps, creating seven expanded cap items that maximize selected Pokemon IVs
- Dry Skin AI fix
...
- The Modern Steel Type patch changes only the battle type-effectiveness chart. On a vanilla chart it changes the Ghost -> Steel and Dark -> Steel multipliers from `0.5x` to `1.0x`; when Fairy Patch is also selected, it edits the Fairy compressed chart instead and verifies Steel -> Fairy remains `2.0x`.
...
- The Item Expansion patch creates contiguous item IDs starting at `0x1D4` / `468`. V2 is still clone-based for item data and graphics, but expanded IDs are stored in an `ITEMBAGV2` overflow table in the tail of the Wi-Fi History save entry instead of consuming vanilla pocket slots. It does not automatically place the new items into scripts, shops, trainers, or maps. If DSPRE Item Standardization is already present, the patcher extends the standardized visible-item scripts through the highest expanded item ID, so Extra TMs, Nature Mints, and Bottle Caps can be used as overworld pickups through the same global-script-ID workflow. Without Item Standardization, use a script edit that sets variable `0x8008` to the expanded item ID before calling the shared pickup script.
- The Extra TMs patch can create up to 60 new item IDs through Item Expansion, displays them as TM93-TM152, and lets each generated TM teach a configured move by move ID or by move name from the loaded ROM's move-name text bank. Extra TMs clone TM01 item behavior and use type-matched TM icon/palette data where possible. The TM/HM bag view is widened to 160 rendered rows by combining vanilla TM/HMs with overflow TM rows. TM93-TM120 use the remaining vanilla personal TM mask bits, while TM121-TM152 use an expanded compatibility table in the synthetic overlay. Fresh rows default to no compatible Pokemon; loading an already-patched ROM prepopulates existing compatibility in the web UI.
- The Nature Mints patch creates all 25 mints through Item Expansion as Medicine-pocket party-use items. Mints change the Pokemon's actual Gen 4 nature by rerolling personality while preserving shiny status, gender, ability slot parity, Unown letter, and Wurmple evolution branch. Spinda spots may change. Eggs, empty slots, and Pokemon already matching the target nature fail without consuming the mint.
- The Bottle Caps patch creates Atk Cap, Def Cap, Spa Cap, SpDef Cap, Spd Cap, HP Cap, and Gold Cap through Item Expansion as Medicine-pocket party-use items. Single-stat caps maximize the matching IV; Gold Cap maximizes all six IVs. Caps fail without consuming the item if the selected stat or stats are already maxed.
...
| Item Expansion `Item_FileID` hook | `0x0207CE78-0x0207CE7F` | `0x00080E78-0x00080E7F` | `0x8` | Literal jump to the synthetic-overlay overflow item-table helper. Vanilla item IDs use the original table; expanded IDs start at `0x1D4`. |
| Item Expansion `Item_Load` hook | `0x0207CF48-0x0207CF4F` | `0x00080F48-0x00080F4F` | `0x8` | Literal jump to the synthetic-overlay item-load helper so expanded IDs can load cloned data/icon/palette members. |
| Item Expansion `Bag_GetPocketForItem` hook | `0x0207D40C-0x0207D417` | `0x0008140C-0x00081417` | `0xC` | Routes expanded item IDs to the `ITEMBAGV2` overflow rows in Wi-Fi History save space with an `r3`-preserving hook because vanilla uses `r3` for the `outMax` pointer. |
| Item Expansion `BagContext_CreateWithPockets` hook | `0x0207D824-0x0207D82B` | `0x00081824-0x0008182B` | `0x8` | Rebuilds the TM/HM bag view from vanilla TM/HMs plus overflow TM rows in synthetic-overlay scratch RAM. |
| Extra TMs `CanPokemonFormLearnTM` hook | `0x02077FE4-0x02077FEB` | `0x0007BFE4-0x0007BFEB` | `0x8` | Routes TM121-TM152 compatibility checks to the Extra TMs expanded compatibility table while preserving vanilla behavior for TM01-TM120. |
| Nature Mints `Pokemon_CheckItemEffects` hook | `0x02096420-0x0209642B` | `0x0009A420-0x0009A42B` | `0xC` | Lets mint items pass or fail party-item eligibility without consuming on eggs, empty slots, or already-matching natures. |
| Nature Mints party item dispatcher hook | `0x020852B8-0x020852BF` | `0x000892B8-0x000892BF` | `0x8` | Routes successful mint item uses to the synthetic-overlay PID reroll and party-menu message helper. |
| Bottle Caps `Pokemon_CheckItemEffects` hook | `0x02096420-0x0209642B` | `0x0009A420-0x0009A42B` | `0xC` | Chained with Nature Mints when both are installed; lets cap items fail without consuming on eggs, empty slots, or already-maxed IVs. |
| Bottle Caps party item dispatcher hook | `0x020852B8-0x020852BF` | `0x000892B8-0x000892BF` | `0x8` | Chained with Nature Mints when both are installed; routes successful cap item uses to the synthetic-overlay IV update and party-menu message helper. |
...
Synthetic-overlay allocations used by this patcher live in `data/weather_sys.narc` member `9`, which is loaded at RAM `0x023C8000` after the DSPRE ARM9 expansion is installed. The shared `SyntheticOverlayAllocator` scans member `9` for an existing ASCII marker first, then dynamically places new payloads into the first large enough aligned zero-filled run. Infinite Candy allocates `chain_candy_red_v1` for party-menu chaining and `inf_redchain_remove_v1` for the Red Chain removal guard. Item Renewal allocates `item_renewal_v12` for its held-item writeback and battle-party display helpers. Item Expansion allocates `ITEMEXPV1` for overflow item lookup. Extra TMs allocates `EXTRATMSV1` for TM/HM behavior and bag display helpers. Nature Mints allocates `NATUREMINTV1` for party-use eligibility, dispatch, PID reroll, and success-message helpers. Bottle Caps allocates `BOTTLECAPV1` for party-use eligibility, dispatch, IV update, and success-message helpers. Nature stat colors allocates `NATSTATCOLOR1` for its summary print helper. Custom overworld sprites allocates `OWTBLXPANDV1` for relocated/expanded overlay 5 appearance tables. Older `chain_candy_start`, `inf_candy_remove_v1`, and older Item Renewal helpers are detected so already-patched ROMs can migrate cleanly.

These current patches do not modify ARM9: No critical hits, critical hit odds, critical damage 1.5x, Remove EV gain, universal infatuation, wild nature filter, Remove Surf/Waterfall checks, VS Seeker QoL, Dry Skin AI fix, Modern Steel Type, player accuracy bypass, and the non-helper portions of Fairy Patch. They use overlays and/or NARC files instead.
...
| Modern paralysis legacy status-hook repair | 16 | `0x79E2-0x79E5` | `0x02242B22-0x02242B25` | `0x001E3FE2-0x001E3FE5` | `0x4` | If an older Modern paralysis patch redirected this battle-script status write, the current patch restores the vanilla `BattleMon_Set` call. Clean/current patches leave it vanilla. |
...
| Modern Steel Type, vanilla chart | 16 | `0x33CAD`, `0x33CC5` | `0x0226EE0D`, `0x0226EE25` | varies by overlay file | `0x1` each | Changes Ghost -> Steel and Dark -> Steel multiplier bytes from `0x05` to `0x0A` without moving the vanilla triplet table. |
| Modern Steel Type, Fairy chart | 16 | `0x33CCE`, `0x33CD2`, `0x33CDA` | `0x0226EE0E`, `0x0226EE12`, `0x0226EE1A` | varies by overlay file | nibble edits | Updates the Fairy compressed type-chart nibbles for Ghost/Dark -> Steel and verifies Steel -> Fairy. |
...
Item Expansion modifies `msgdata/pl_msg.narc` members `391`, `392`, `393`, and `394` to add item descriptions, names, article names, and plural names for generated IDs starting at entry `468`. Ordinary clone rows do not append item data or icon NARC members; expanded rows point to cloned vanilla members through the overflow table. Nature Mints and Bottle Caps are the current exceptions: Nature Mints appends one shared mint item-data member, one mint icon, and six mint palettes; Bottle Caps appends one shared cap item-data member, one cap icon, and two cap palettes. Runtime inventory storage uses the last `0x300` bytes before the Wi-Fi History save-entry checksum for an `ITEMBAGV2` marker and 128 expanded `BagItem` rows. If DSPRE Item Standardization is already present, the standardized visible-item script file is extended to include expanded item IDs. Extra TMs use the same text banks and add per-row move mappings in the Extra TMs helper. Nature Mints writes its party-menu success text to `msgdata/pl_msg.narc` member `453`, entry `260`; Bottle Caps writes entries `261-267`. Clean Platinum personal entries are 44 bytes, so Extra TMs does not extend `pl_personal.narc` entries to a `0x64-0x67` field. Instead, TM93-TM120 use the existing fourth vanilla TM mask and TM121-TM152 use the Extra TMs synthetic-overlay compatibility table.
