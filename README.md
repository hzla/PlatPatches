...
- The Extra TMs patch can create up to 60 new item IDs through Item Expansion, displays them as TM93-TM152, and lets each generated TM teach a configured move by move ID or by move name from the loaded ROM's move-name text bank. Extra TMs clone TM01 item behavior and use type-matched TM icon/palette data where possible. The TM/HM bag view is widened to 160 rendered rows by combining vanilla TM/HMs with overflow TM rows. TM93-TM120 set the remaining vanilla personal TM mask bits for every personal entry, while TM121-TM152 use an expanded compatibility table that currently defaults every personal entry to compatible.
...
Item Expansion modifies `msgdata/pl_msg.narc` members `391`, `392`, `393`, and `394` to add item descriptions, names, article names, and plural names for generated IDs starting at entry `468`. It does not append item data or icon NARC members in V2; expanded rows point to cloned vanilla members through the overflow table. Runtime inventory storage uses the last `0x300` bytes before the Rankings save-entry checksum for an `ITEMBAGV2` marker and 128 expanded `BagItem` rows. Visible-item pickup scripts are not rewritten by this patch; use DSPRE Item Standardization or a script edit that sets `0x8008` to the expanded item ID when placing expanded overworld pickups. Extra TMs use the same text banks and add per-row move mappings in the Extra TMs helper. Clean Platinum personal entries are 44 bytes, so Extra TMs does not extend `pl_personal.narc` entries to a `0x64-0x67` field. Instead, TM93-TM120 use the existing fourth vanilla TM mask and TM121-TM152 use the Extra TMs synthetic-overlay compatibility table.
