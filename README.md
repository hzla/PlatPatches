...
- The Item Expansion patch creates contiguous item IDs starting at `0x1D4` / `468`. V2 is still clone-based for item data and graphics, but expanded IDs are stored in an `ITEMBAGV2` overflow table in the tail of the Rankings save entry instead of consuming vanilla pocket slots. It does not automatically place the new items into scripts, shops, trainers, or maps, and it leaves visible-item scripts unchanged. For overworld pickups, use DSPRE's Item Standardization flow or a script edit that sets variable `0x8008` to the expanded item ID before calling the shared pickup script.
...
| Item Expansion `Bag_GetPocketForItem` hook | `0x0207D40C-0x0207D417` | `0x0008140C-0x00081417` | `0xC` | Routes expanded item IDs to the `ITEMBAGV2` overflow rows in Rankings save space with an `r3`-preserving hook because vanilla uses `r3` for the `outMax` pointer. |
...
Item Expansion modifies `msgdata/pl_msg.narc` members `391`, `392`, `393`, and `394` to add item descriptions, names, article names, and plural names for generated IDs starting at entry `468`. It does not append item data or icon NARC members in V2; expanded rows point to cloned vanilla members through the overflow table. Runtime inventory storage uses the last `0x300` bytes before the Rankings save-entry checksum for an `ITEMBAGV2` marker and 128 expanded `BagItem` rows. If DSPRE Item Standardization is already present, the standardized visible-item script file is extended to include expanded item IDs. Extra TMs use the same text banks and add per-row move mappings in the Extra TMs helper. Clean Platinum personal entries are 44 bytes, so Extra TMs does not extend `pl_personal.narc` entries to a `0x64-0x67` field. Instead, TM93-TM120 use the existing fourth vanilla TM mask and TM121-TM152 use the Extra TMs synthetic-overlay compatibility table.
