...
- The Item Renewal patch snapshots the player's held items as Pokemon enter battle, then writes a saved item back during battle cleanup only if that party slot currently has no held item. Item loss and swaps still affect the current battle; consumed or lost items are not restored until battle ends, so switching out and back in will not bring an item back early. Newly gained held items are kept. Link, tag, and 2-vs-2 battles are skipped to avoid touching partner or remote parties.
...
| Item Renewal snapshot hook | 16 | `0x16B54-0x16B57` clean, `0x16B5C-0x16B5F` pkaizo | `0x02251C94-0x02251C97` clean, `0x02251C9C-0x02251C9F` pkaizo | `0x001F315C-0x001F315F` pkaizo | `0x4` | Hooks `BattleSystem_InitBattleMon` to snapshot player held items into the synthetic overlay. |
| Item Renewal end-of-battle restore hook | 16 | `0x15620-0x15623` clean, `0x15628-0x1562B` pkaizo | `0x02250760-0x02250763` clean, `0x02250768-0x0225076B` pkaizo | `0x001F1C28-0x001F1C2B` pkaizo | `0x4` | Hooks `BattleControllerPlayer_EndFight` to restore saved player held items after battle cleanup. |
