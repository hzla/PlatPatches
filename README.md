...
- Item Renewal, restoring consumed/lost player held items only after battle ends
...
- The Item Renewal patch preserves the player's saved held items by masking player-side held-item writeback messages instead of restoring from a separate snapshot table. Item loss still affects the current battle: consumed or lost items mark the battle context knocked-off mask, so switching out and back in, opening the battle party screen, and viewing the battle party summary all keep the item absent until battle ends.
...
| Item Renewal held-item writeback hook | 16 | `0x213A4-0x213A7` clean, `0x213C0-0x213C3` pkaizo | `0x0225C4E4-0x0225C4E7` clean, `0x0225C500-0x0225C503` pkaizo | `0x001FD9C0-0x001FD9C3` pkaizo | `0x4` | Masks player-side held-item writeback messages and marks the battle context knocked-off mask when a player item is consumed or lost. |
| Item Renewal battle party display hook | 13 | `0x14D8-0x14DB` | `0x022210F8-0x022210FB` | `0x001C40D8-0x001C40DB` | `0x4` | Masks the battle party held-item cache so item icons and summary item text stay hidden mid-battle after consumption/loss. |
